...
public interface LeaderboardSnapshotRepository extends JpaRepository<LeaderboardSnapshot, Long> {
    ...
    @Query("SELECT COUNT(DISTINCT dla2.user.id) + 1 " +
            "FROM DailyLearningActivity dla2 " +
            "WHERE dla2.activityDate BETWEEN :start AND :end " +
            "AND dla2.user.id != :userId " +
            "HAVING SUM(dla2.xpEarned) > (SELECT COALESCE(SUM(dla1.xpEarned), 0) FROM DailyLearningActivity dla1 WHERE dla1.user.id = :userId AND dla1.activityDate BETWEEN :start AND :end)")
    Optional<Long> findUserRealTimeRank(@Param("userId") Long userId, @Param("start") LocalDate start, @Param("end") LocalDate end);
