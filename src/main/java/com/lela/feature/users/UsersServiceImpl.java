...
import com.lela.feature.language.LanguageRepository;
...
public class UsersServiceImpl implements UsersService {
    ...
    public List<UsersResponse> findAll() {
        // Lấy danh sách tất cả các thực thể người dùng từ DB và ánh xạ (map) sang danh sách DTO phản hồi
    ...
    public UsersResponse create(UsersCreateRequest request) {
        ...
        // Nếu request có truyền nativeLanguageId, tiến hành kiểm tra tồn tại và gán vào entity
        if (request.getNativeLanguageId() != null) {
            Language nativeLang = languageRepository.findById(request.getNativeLanguageId())
                    .orElseThrow(() -> new NotFoundExeception("Not found Language with id " + request.getNativeLanguageId()));
            entity.setNativeLanguage(nativeLang);
        }

        // Nếu request có truyền targetLanguageId, tiến hành kiểm tra tồn tại và gán vào entity
        if (request.getTargetLanguageId() != null) {
            Language targetLang = languageRepository.findById(request.getTargetLanguageId())
                    .orElseThrow(() -> new NotFoundExeception("Not found Language with id " + request.getTargetLanguageId()));
    ...
    public UsersResponse patch(Long id, UsersPatchRequest request) {
        // Tìm kiếm thông tin người dùng hiện tại trong cơ sở dữ liệu, ném ngoại lệ nếu không tìm thấy
        Users entity = repository.findById(id)
                .orElseThrow(() -> new NotFoundExeception("Not found Users with id " + id));

        // Cập nhật từng trường thông tin nếu giá trị trong request khác null (Null-safe partial update)
        if (request.getUsername() != null) entity.setUsername(request.getUsername());
        if (request.getEmail() != null) entity.setEmail(request.getEmail());
        if (request.getPasswordHash() != null) entity.setPasswordHash(request.getPasswordHash());
        ...
        if (request.getDailyGoalCards() != null) entity.setDailyGoalCards(request.getDailyGoalCards());
        if (request.getXpTotal() != null) entity.setXpTotal(request.getXpTotal());
        if (request.getStreakCurrent() != null) entity.setStreakCurrent(request.getStreakCurrent());
        if (request.getStreakLongest() != null) entity.setStreakLongest(request.getStreakLongest());
        if (request.getLastActivityDate() != null) entity.setLastActivityDate(request.getLastActivityDate());
        if (request.getLastActiveAt() != null) entity.setLastActiveAt(request.getLastActiveAt());
        if (request.getEmailVerifiedAt() != null) entity.setEmailVerifiedAt(request.getEmailVerifiedAt());
        if (request.getDeletedAt() != null) entity.setDeletedAt(request.getDeletedAt());
